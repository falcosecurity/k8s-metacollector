...
const (
	...
	eventSentKey       = "event_subscribers_sent"
...
var (
	// generatedEvents is a prometheus counter metrics which holds the total
	// number of events generated per collector. It has two labels. name label refers
	// to the collector name and type label refers to the event type, i.e.
	// added, updated, deleted.
	generatedEvents = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace: consts.MetricsNamespace,
		Subsystem: collectorSubsystem,
		Name:      eventSentKey,
		Help: "Total number of events generated by the collector destined to subscribers name label refers to the " +
			"collector name and type label refers to the event type, i.e. create, update, delete, generic",
	}, []string{"name", "type"})
...
func init() {
	...
	metrics.Registry.MustRegister(generatedEvents)
	metrics.Registry.MustRegister(ingestedEvents)
}

type generatedEventsMetrics struct {
...
func newGeneratedEventsMetcrics(name string) generatedEventsMetrics {
	...
	updateCounter.Add(0)
	deleteCounter := generatedEvents.WithLabelValues(name, labelUpdate)
	deleteCounter.Add(0)

	return generatedEventsMetrics{
