...
func (r *ReplicasetCollector) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	var err error
	var rs = newPartialReplicaset
	var rsRes *events.GenericResource
	var ok, replicasetDeleted, updated bool
	...
	if k8sApiErrors.IsNotFound(err) {
		// When the k8s resource is deleted we need to remove it from the local cache.
		if _, ok = r.Cache.Get(req.String()); ok {
			logger.Info("marking resource for deletion")
			replicasetDeleted = true
		} else {
	...
	currentNodes, err := r.Nodes(ctx, logger, &rs.ObjectMeta)
	if err != nil {
		return ctrl.Result{}, err
	}
...
// Nodes returns all the nodes where pods related to the current replicaset are running.
func (r *ReplicasetCollector) Nodes(ctx context.Context, logger logr.Logger, meta *metav1.ObjectMeta) (fields.Nodes, error) {
	pods := corev1.PodList{}

	if err := r.List(ctx, &pods, client.InNamespace(meta.Namespace), client.MatchingFields{
...
func (r *ReplicasetCollector) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1.ReplicaSet{}, builder.OnlyMetadata).
